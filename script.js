...
function drawDiagram(result) {
  ...
  segments.forEach((seg, i) => {
    ...
    if (seg.type === 'b' && seg.anchor !== undefined) {
      addText(svg, 158, cursorY + h / 2 + 3, `sisipan @${seg.anchor}m`, 'start', 7.5, '#F2B705');
    }
    cursorY += h;
  });
...
function buildSegments(result) {
  // Insert type-B (shorter) pipes exactly at candidate marker depths
  // 5, 8, 11, 14, 17... (m), matched to the computed nB count. Type-A
  // pipes fill the string up to each marker before the insertion.
  // Any leftover A pipes (once all nB markers are used) go last, before
  // the core barrel.
  const anchors = [];
  for (let m = 5, i = 0; i < result.nB; m += 3, i++) anchors.push(round2(m));

  const segs = [];
  let remainingA = result.nA;
  let cumulative = 0;

  anchors.forEach(anchor => {
    while (remainingA > 0 && cumulative + result.lenA <= anchor + 1e-6) {
      segs.push({ type: 'a', len: result.lenA, label: `${result.lenA.toFixed(1)}m` });
      cumulative += result.lenA;
      remainingA--;
    }
    segs.push({ type: 'b', len: result.lenB, label: `${result.lenB.toFixed(1)}m`, anchor });
    cumulative += result.lenB;
  });
...
function drawLegend(svg, y) {
  ...
  addText(svg, W_FALLBACK(svg), y - 18, 'Label "sisipan @Xm" = pipa B disisipkan di titik meteran tsb', 'middle', 7.5, '#8D9497');
}
