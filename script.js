...
function drawDiagram(result) {
  ...
  let cursorY = groundY;
  const segBoundariesDepth = [0];
  const segBoundariesPx = [groundY];
  segments.forEach((seg, i) => {
    ...
    cursorY += h;
    segBoundariesDepth.push(segBoundariesDepth[segBoundariesDepth.length - 1] + seg.len);
    segBoundariesPx.push(cursorY);
  });

  // Optional pipe-insertion candidate markers: 5, 8, 11, 14, 17... m (informational only)
  drawOptionalMarkers(svg, segments, segBoundariesDepth, segBoundariesPx, W);
...
function drawOptionalMarkers(svg, segments, boundDepth, boundPx, W) {
  const totalDepth = boundDepth[boundDepth.length - 1];
  const markers = [];
  for (let m = 5; m <= totalDepth + 1e-6; m += 3) markers.push(round2(m));

  markers.forEach(depth => {
    // find the segment this depth falls in
    let idx = 0;
    while (idx < boundDepth.length - 2 && depth > boundDepth[idx + 1]) idx++;
    const segLen = boundDepth[idx + 1] - boundDepth[idx];
    const frac = segLen > 0 ? (depth - boundDepth[idx]) / segLen : 0;
    const y = boundPx[idx] + frac * (boundPx[idx + 1] - boundPx[idx]);
    ...
    line.setAttribute('stroke', '#F2B705');
    ...
    dot.setAttribute('cx', 58);
    dot.setAttribute('cy', y);
    dot.setAttribute('r', 2.2);
    dot.setAttribute('fill', '#F2B705');
    svg.appendChild(dot);

    addText(svg, 52, y + 3, `${depth}m*`, 'end', 7.5, '#F2B705');
  });
}
...
function drawLegend(svg, y) {
  ...
  addText(svg, W_FALLBACK(svg), y - 18, '- - - * = titik opsional penambahan pipa (tiap +3 m mulai 5 m)', 'middle', 7.5, '#8D9497');
}
