...
/* ---------- core calculation ---------- */
// Pipe B (short rod) is a temporary drive/connector piece used only to
// seat pipe A into the ground — it is withdrawn afterward. In practice
// it is used at most twice per hole, always at the top of the string.
const MAX_PIPE_B = 2;
...
function computeStickUp(targetDepth, cb, lenA, lenB, minStickUp) {
  ...
  // Fill primarily with pipe A.
  let nA = Math.floor(pipeNeeded / lenA);
  let remaining = pipeNeeded - nA * lenA;
  let nB = 0;
  let totalPipe = nA * lenA;

  // Cover the remainder with pipe B, capped at MAX_PIPE_B.
  while (remaining > 1e-9 && nB < MAX_PIPE_B) {
    nB++;
    totalPipe += lenB;
    remaining -= lenB;
  }
  // Safety net: if the cap wasn't enough to cover the remainder
  // (only possible with unusual custom lengths), add one more pipe A.
  if (remaining > 1e-9) {
    nA++;
    totalPipe += lenA;
  }
  ...
  // in the field, so keep adding pipe until stick up clears the minimum
  // practical threshold. Pipe B is used first, but never beyond its cap —
  // once that cap is reached, pipe A is added instead.
  let adjustments = 0;
  let addedA = 0;
  let addedB = 0;
...
function runCalculation() {
  ...
  if (result.adjustments > 0) {
    ...
    const parts = [];
    if (result.addedB > 0) parts.push(`${result.addedB} pipa B (${lenB.toFixed(1)} m)`);
    if (result.addedA > 0) parts.push(`${result.addedA} pipa A (${lenA.toFixed(1)} m)`);
    els.adjustNote.textContent = `Disesuaikan otomatis: ditambahkan ${parts.join(' + ')} karena stick up awal berada di bawah batas minimum ${fmt(result.minStickUp)} m.`;
  } else {
    els.adjustNote.hidden = true;
  }
...
function buildSegments(result) {
  // Pipe B is a temporary drive/connector piece used only near the
  // surface to seat pipe A into the ground, then withdrawn — so in the
  // illustration it always sits at the very top of the string (closest
  // to surface), never spliced between pipe A rods or the core barrel.
  const segs = [];
...
function drawLegend(svg, y) {
  ...
  addText(svg, W_FALLBACK(svg), y - 18, 'Pipa B: alat bantu dorong di permukaan, maks. 2, ditarik keluar setelah pipa A masuk', 'middle', 7.5, '#8D9497');
}

function W_FALLBACK(svg) {
  const vb = svg.getAttribute('viewBox').split(' ');
  return parseFloat(vb[2]) / 2;
}
